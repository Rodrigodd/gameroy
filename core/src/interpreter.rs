...
impl Interpreter<'_> {
    ...
    pub fn next_interpret_cycle_count(&self) -> u8 {
        ...
        if op == 0xcb {
            let op = self.0.read(add16(self.0.cpu.pc, 1));
            count += consts::CB_CLOCK[op as usize];
        } else {
            count += consts::CLOCK[op as usize];
            // if a jump happens, the number of clocks incresase
            if self.will_jump_to().is_some() {
                count += match op {
                    0xC3 => {
                        // JP $aaaa
                        0
